...
Read in the data
```{r, message = FALSE}
ds2 <- read_csv(here("data/max_bf.csv"))
ds2_male <- read_csv(here("data/max_bf_males.csv"))
ds2_female <- read_csv(here("data/max_bf_females.csv"))
```
...
write_csv(family_bf2, path = here("tables/bite-force-by-family.csv"))
```
...
```{r}
# Stick the datasets together
both1 <- rbind(ds2_female, ds2_male)
```
...
#write_csv(family_both1, path = here("tables/meansd/bite-force-by-family-sex.csv"))
```
...
#write_csv(higher_both1, path = here("tables/meansd/bite-force-by-higher-sex.csv"))
```
...
# Save the plot
# ggsave(here("figures/meansd/variable-density-plots.png"), dpi = 900, height = 8)
```
## By sexes
...
# Save the plot
# ggsave(here("figures/meansd/variable-density-plots-sex.png"), dpi = 900, height = 8)
```
