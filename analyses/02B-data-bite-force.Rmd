...
dietplot <-
  ...
  geom_jitter(alpha = 0.5, width = 0.2) +
  geom_point(data = diet_bf1, aes(y = MeanBF), size = 3, col = "red") +
  geom_errorbar(data = diet_bf1, 
                aes(y = MeanBF, ymin = MeanBF - SeBF, ymax = MeanBF + SeBF), 
                show.legend = FALSE, width = 0.2, col = "red") +
  scale_colour_manual(values = sf_colours) +
  coord_flip() +
  theme_bw(base_size = 14) +
  xlab("") +
...
arbplot <-
  ...
  geom_jitter(alpha = 0.5, width = 0.2) +
  geom_point(data = arb_bf1, aes(y = MeanBF), size = 3, col = "red") +
...
Bite force for strict arboreality
```{r}
arb_strict_bf1 <-
  ds1 %>%
  filter(!is.na(arboreal_strict)) %>%
  group_by(arboreal_strict) %>%
  summarise(MeanBF = round(mean(log(max_bf)), digits = 3),
            SeBF = round(sd(log(max_bf))/sqrt(n()), digits = 3),
            n())
...
#write_csv(arb_strict_bf1, path = here("tables/bite-force-by-arboreal_strict.csv"))
```

```{r}
# Remove NAs
ds1_arb_strict <- filter(ds1, !is.na(arboreal_strict))

arbplot_strict <-
  ggplot(ds1_arb_strict, aes(y = log(max_bf), x = arboreal_strict)) + 
  geom_jitter(alpha = 0.5, width = 0.2) +
  geom_point(data = arb_strict_bf1, aes(y = MeanBF), size = 3, col = "red") +
  geom_errorbar(data = arb_strict_bf1, 
                aes(y = MeanBF, ymin = MeanBF - SeBF, ymax = MeanBF + SeBF), 
                show.legend = FALSE, width = 0.2, col = "red") +
  scale_colour_manual(values = sf_colours) +
  coord_flip() +
...
#write_csv(arb_bf1, path = here("tables/bite-force-by-fossorial.csv"))
```
...
fosplot <-
  ...
  geom_jitter(alpha = 0.5, width = 0.2) +
  geom_point(data = fos_bf1, aes(y = MeanBF), size = 3, col = "red") +
...
Bite force for strict fossoriality
```{r}
fos_strict_bf1 <-
  ds1 %>%
  filter(!is.na(fossorial_strict)) %>%
  group_by(fossorial_strict) %>%
  summarise(MeanBF = round(mean(log(max_bf)), digits = 3),
            SeBF = round(sd(log(max_bf))/sqrt(n()), digits = 3),
            n())
...
```{r}
# Remove NAs
ds1_fos_strict <- filter(ds1, !is.na(fossorial_strict))

fosplot_strict <-
  ggplot(ds1_fos_strict, aes(y = log(max_bf), x = fossorial_strict)) + 
  geom_jitter(alpha = 0.5, width = 0.2) +
  geom_point(data = fos_strict_bf1, aes(y = MeanBF), size = 3, col = "red") +
  geom_errorbar(data = fos_strict_bf1, 
                aes(y = MeanBF, ymin = MeanBF - SeBF, ymax = MeanBF + SeBF), 
                show.legend = FALSE, width = 0.2, col = "red") +
  scale_colour_manual(values = sf_colours) +
  coord_flip() +
...
acrodontplot <-
  ...
  geom_jitter(alpha = 0.5, width = 0.2) +
  geom_point(data = acrodont_bf1, aes(y = MeanBF), size = 3, col = "red") +
...
higherplot + (dietplot/arbplot) + (fosplot/ acrodontplot)

# Save plot
#ggsave(here("figures/bite-force-by-ecology.png"), width = 10, height = 5)
```
