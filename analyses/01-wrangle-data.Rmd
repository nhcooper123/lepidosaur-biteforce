...
bfdata1 %>%
filter(!is.na(BF.MAX)) %>%
  select(BinomialReptileDatabase) %>%
  distinct()
...
ecodata1 <- 
  ...
  mutate(arboreal = case_when(Lifestyle == "Arboreal" | Lifestyle == "Arboreal/Saxicolous" |
   ...
   mutate(arboreal_strict = case_when(Lifestyle == "Arboreal"  ~ "Arboreal", Lifestyle != "Arboreal" ~ "Other")) %>%
   ...
   mutate(fossorial_strict = case_when(Lifestyle == "Fossorial"  ~ "Fossorial",
...
max_bf <- 
   ...
   dplyr::select(BinomialReptileDatabase, Sex, Family, HigherTaxonomy, tips,
          ...
          arboreal, fossorial, Diet, acrodont, arboreal_strict, fossorial_strict)
