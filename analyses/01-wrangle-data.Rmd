...
bfdata1 <- 
  ...
  # Note we use sum, not + so that if there is no SE or SD this outputs the mean.
  ...
  mutate(bf_plus_se = sum(BF, BF.SE, na.rm = TRUE)) %>%
  mutate(svl_plus_se = sum(SVL, SVL.SE, na.rm = TRUE)) %>%
  mutate(bm_plus_se = sum(BM, BM.SE, na.rm = TRUE)) %>%
  mutate(hl_plus_se = sum(HL, HL.SE, na.rm = TRUE)) %>%
  mutate(hw_plus_se = sum(HW, HW.SE, na.rm = TRUE)) %>%
  mutate(hh_plus_se = sum(HH, HH.SE, na.rm = TRUE)) %>%
  mutate(ljl_plus_se = sum(LJL, LJL.SE, na.rm = TRUE)) %>%
